...
function formatElementRef(element) {
  ...
  if (element.className && typeof element.className === 'string') {
    ...
    if (classes.length > 0 && classes[0]) { ref += `.${classes.join('.')}`; }
  }
  return ref;
}
...
function getNeighbours(element) {
  ...
  return { parent: parent ? formatElementRef(parent) : null, previousSibling: previousSibling ? formatElementRef(previousSibling) : null, nextSibling: nextSibling ? formatElementRef(nextSibling) : null, childrenCount: children ? children.length : 0, childrenTypes: children ? getChildrenTypes(children) : [] };
}
